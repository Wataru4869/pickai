...
export async function generateMetadata({params}:{params:Promise<{id:string}>}){const {id}=await params;return Object.hasOwn(map,id)?{title:"用途別AIの選び方 | AI選び",robots:{index:false,follow:true},alternates:{canonical:["translation","writing"].includes(id)?`/category/${id}`:`/categories/${map[id]}`}}:{};}
