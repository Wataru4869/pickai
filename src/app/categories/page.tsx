...
export default function CategoriesPage(){return <div className={styles.page}><Header/><main>
  ...
  </section><section className={styles.decisionBand}><div><p className={styles.japaneseEyebrow}>まだ用途が決まらないとき</p><h2>3つの質問から候補の入口を確認</h2><p>保存済みの選定ルールで候補を整理します。現在性能を保証するランキングではありません。</p></div><div><a href="/recommend">おすすめ候補を確認</a><a href="/models">AIツール一覧を見る</a></div></section></div>
  </main><Footer/></div>}
