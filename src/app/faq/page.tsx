...
const FAQ_ITEMS = [
  {
    ...
    questions: [
      ...
      {
        q: "どのくらいの頻度で更新されますか？",
        a: "客観情報は公式ソースと確認日を記録して更新します。独自評価はモデル識別子、プロンプト、raw回答、採点根拠が揃った場合だけ更新します。過去の評価時点は履歴ページで区別します。",
      },
      {
        q: "特定の企業と提携していますか？",
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        a: "安全性ページでは、入力・出典・公開条件の確認手順を案内しています。旧スコアによる安全認定は行いません。業務利用では各社の現行規約、データ利用、保持、管理機能を公式資料で確認してください。",
      },
      {
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        a: "再現用ログが不足している保存評価から、現在のリスクの高低は判定できません。生成物を公開・商用利用する前に、利用規約、権利帰属、類似性を個別に確認してください。",
      },
    ],
  },
  {
    ...
    questions: [
      {
        q: "総合スコアの算出方法は？",
        a: "総合スコアは過去の保存値です。現在の点数は算出していません。保存資料の評価構成と、raw回答・採点ログがない限界は「評価方法論」で説明しています。",
      },
      {
        q: "DeepSeek・Qwen・Kimiの点数がないのはなぜですか？",
        a: "DeepSeek・Qwen・KimiはモデルAPI候補として掲載しています。公式の製品情報と独自採点は別です。同条件の実行ログと採点根拠が揃っていないため、現在の点数は掲載していません。",
      },
      {
        q: "2モデル比較ページとは何ですか？",
        a: "既存の2サービスを、公式機能・プラン・利用条件の出典と確認日で比べるページです。古いテストの勝敗で現在の購入推奨を決めません。",
      },
    ],
  },
];
      ...
      <Block>
        <div className="border border-[var(--border)] rounded p-4 text-center">
          ...
          <p className="text-[12px] text-[var(--text-sub)] mb-3">
            保存済みルールから用途に合う候補を整理します（現在性能の保証ではありません）
          </p>
