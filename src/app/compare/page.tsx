...
export default function Page() { const title="何に使うか決めてから、AIを2つ比べる"; return <FactsLayout title={title} intro="比較するAIが決まっていなくても大丈夫です。まず用途を選び、候補を2つに絞って、料金・機能・利用条件を確認できます。" eyebrow="AIを比較する" breadcrumbs={[{label:"トップ",href:"/"},{label:"AIを比較"}]}>
  <section className={styles.start} aria-labelledby="compare-purpose-title"><div className={styles.sectionHeading}><p>最初の入口</p><h2 id="compare-purpose-title">何に使いたいですか？</h2><span>近い目的を選ぶと、確認すべき比較へ進めます。</span></div><div className={styles.purposeGrid}>{purposeLinks.map((item,index) => <a key={item.label} href={item.href} data-analytics-event="internal_cta_click" data-source-page="/compare" data-cta-type="purpose" data-cta-position={`compare_purpose_${index + 1}`} data-destination-id={item.href}><strong>{item.label}</strong><span>{item.detail}</span><b aria-hidden="true">比較を見る →</b></a>)}</div></section>
  <section className={styles.popular} aria-labelledby="popular-comparisons-title"><div className={styles.sectionHeading}><p>代表的な比較</p><h2 id="popular-comparisons-title">迷ったときに確認しやすい3組</h2><span>勝敗や人気順位ではなく、用途と確認条件で整理しています。</span></div><div className={styles.popularGrid}>{popular.map((item,index) => { const [a,b]=item.ids; const href=`/compare/${a}-vs-${b}`; return <article key={`${a}-${b}`}><p>{item.theme}</p><h3>{modelNames[a]} と {modelNames[b]}</h3><dl><div><dt>{modelNames[a]}</dt><dd>{item.first}</dd></div><div><dt>{modelNames[b]}</dt><dd>{item.second}</dd></div></dl><div className={styles.difference}><strong>見る違い</strong><span>{item.difference}</span></div><a href={href} data-analytics-event="internal_cta_click" data-source-page="/compare" data-cta-type="compare" data-cta-position={`compare_featured_${index + 1}`} data-destination-id={href}>この2つを比較する</a></article>; })}</div></section>
