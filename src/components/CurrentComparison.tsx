  ...
  return <section id="current-comparison" className={styles.section} aria-labelledby="current-title">
    ...
    <div className={styles.grid}>
      {visible.map(p => {
        ...
        return <article key={p.id} className={`${styles.card} ${active ? styles.selected : ""}`} data-group={p.group}>
          ...
          {p.provider.source && <p className={styles.provider}>{p.provider.text}</p>}
          {p.product.source && <p className={styles.product}>{p.product.text}</p>}
          <p className={styles.question}>{p.question}</p>
          <dl className={styles.facts}>{p.features.source && <div><dt>公式の機能情報</dt><dd><Fact fact={p.features} /></dd></div>}{p.free.source && <div><dt>無料プラン</dt><dd><Fact fact={p.free} /></dd></div>}</dl>
          <details className={styles.details}><summary>提供条件・確認範囲を見る</summary>{p.availability.source && <p><Fact fact={p.availability} /></p>}{p.product.source && <p><Fact fact={p.product} /></p>}<p>用途の問いかけは編集上の分類です。品質・成功率・安全性の優劣を示す点数ではありません。</p></details>
          <div className={styles.cardBottom}><span className={styles.unrated}>公式情報で比較</span><button type="button" aria-label={`${p.name}を${active ? "比較から外す" : "比較に追加"}`} aria-pressed={active} disabled={unavailable} onClick={() => toggle(p)}>{active ? "✓ 比較中" : "＋ 比較に追加"}</button></div>
    ...
    {chosen.length >= 2 && <section id="selected-comparison" className={styles.comparison} aria-labelledby="selected-title">
      ...
      <div className={styles.scroll} tabIndex={0} role="region" aria-label="選択した候補の比較表。横スクロール可能">
        <table><caption>同じ項目について、出典を確認できた公式情報だけを比較します。</caption><thead><tr><th scope="col">確認すること</th>{chosen.map(p => <th scope="col" key={p.id}>{p.name}</th>)}</tr></thead><tbody>
          {([ ["product", "モデル / 製品"], ["features", "機能"], ["free", "無料プラン"], ["price", "掲載プラン料金"], ["availability", "提供条件"] ] as const).filter(([field]) => chosen.every(p => Boolean(p[field].source))).map(([field,label]) => <tr key={field}><th scope="row">{label}</th>{chosen.map(p => <td key={p.id}><Fact fact={p[field]} /></td>)}</tr>)}
          <tr><th scope="row">評価方法</th>{chosen.map(p => <td key={p.id}>公式情報と同じ作業での試用を分けて判断<br /><small>過去の保存スコアは現在の比較に使用しません</small></td>)}</tr>
          <tr><th scope="row">次に確かめる</th>{chosen.map(p => <td key={p.id}><a href={p.guide}>選び方ガイド →</a></td>)}</tr>
        </tbody></table>
      </div>
    </section>}
    <div className={styles.evidence}><strong>現在の比較は、公式情報を基準にしています。</strong><p>2026年3月の保存値は、新モデルの性能を測った結果ではありません。現在は出典付きの機能・料金・提供条件と、同じ作業で試す手順を分けて掲載します。</p><a href="/evaluations/2026-03">3月の保存スコアを見る →</a><a href="/methodology">評価の根拠と限界 →</a></div>
  </section>;
}
